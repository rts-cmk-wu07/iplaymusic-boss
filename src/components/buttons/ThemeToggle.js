...
const ThemeToggle = () => {
  ...
  const [themeToggle, setThemeToggle] = useState(
    localStorage.getItem("theme") === "dark" ? "dark" : localStorage.getItem("theme") === "light" ? "light" : "auto"
  );
  ...
  darkTheme.addEventListener("change", (e) => {
    if (e.matches) {
      ...
      themeToggle === "auto" && document.documentElement.classList.remove("dark");
    }
  });
  ...
  return (
    <motion.button
      ...
      animate={themeToggle === "light" ? "light" : themeToggle === "dark" ? "dark" : "auto"}
      whileTap={{ scale: 0.9 }}
      onClick={toggleTheme}
    >
      <motion.div className="absolute w-6 h-6 flex items-center justify-center">
        <motion.div className="absolute w-4 h-4 border-4 rounded-full border-primary" variants={sun.circle}></motion.div>
        {raysArray.map((i) => (
          <motion.span
      ...
      <motion.div className="absolute w-6 h-6 text-primary" variants={moon}>
        <FeatherIcon icon="moon" strokeWidth={themeToggle === "auto" ? "4" : "4"} className="w-full h-full" />
      </motion.div>
    </motion.button>
  );
};
