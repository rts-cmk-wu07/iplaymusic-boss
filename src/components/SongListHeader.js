...
const SongListHeader = ({ playlist }) => {
  ...
  const regexExp = /\p{Extended_Pictographic}/gu;
  if (titleRef.current) {
    titleRef.current.innerHTML = titleRef.current.innerHTML.replaceAll(
      regexExp,
  ...
  return (
    <header className="pb-2 mb-2">
      ...
      <section className="mt-4">
        ...
        >
          <h1 className="heading gradient-text" ref={titleRef}>
            {playlist.name}
          </h1>
        </motion.div>
