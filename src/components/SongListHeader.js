...
import PlaylistSearch from "./subcomponents/PlaylistSearch";

const SongListHeader = ({ playlist }) => {
  const { songData } = useContext(SongContext);
  const isSongPlaying = playlist?.tracks?.items?.some((song) => song?.track?.id === songData?.id);
    ...
    <header className="pb-2 mb-2">
          ...
          <>
            <motion.div
              ...
              <motion.div
                ...
                <img src={playlist.images && playlist.images[0].url} alt="" className="w-12 h-12 object-cover rounded-full" />
      ...
      <section className="mt-4">
        ...
        <p className="text-gray-400 dark:text-gray-600 text-sm">
          By{" "}
          <Link className="font-semibold text-primary z-50" to={`/user/${playlist?.owner?.id}`}>
            {playlist.owner && playlist.owner.display_name}
          </Link>
        </p>
        {playlist.description && <p className="my-4 text-black/75 dark:text-white/75 text-base">{playlist.description}</p>}
        <div className="flex justify-between items-center text-black/75 dark:text-white/75 text-xs">
          ...
          {playlist.followers && <p>{playlist.followers.total.toLocaleString()} followers</p>}
        </div>
      </section>
