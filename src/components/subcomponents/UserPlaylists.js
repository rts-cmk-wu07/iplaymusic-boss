...
const UserPlayLists = ({ id, name }) => {
  const { data, loading } = useFetch(`https://api.spotify.com/v1/users/${id}/playlists`);
  console.log(data);
  return (
    <div className="flex flex-col w-full my-6">
      <h2 className="text-3xl text-additional dark:text-white font-semibold mb-4 px-6">{name}'s Playlists</h2>
      {!loading ? (
