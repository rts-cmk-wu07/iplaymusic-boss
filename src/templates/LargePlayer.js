...
import { IoChevronDown, IoPlaySkipBack, IoPlayBack, IoPlayForward, IoPlayCircle, IoPlaySkipForward, IoPauseCircle } from "react-icons/io5";
...
const LargePlayer = ({ isOpen, setIsOpen, song, isPlaying, controls, songProgress, setSongProgress }) => {
  const { data } = useFetch(`https://api.spotify.com/v1/artists/${song?.artists[0].id}/`);
  return (
    <AnimatePresence>
      {isOpen && (
            ...
            <div className="px-6">
              ...
              >
                <motion.img
                  ...
                  alt="album art"
                  className="h-64 w-64 rounded-full user-select-none"
                />
              </motion.div>
              <motion.div>
                ...
                <motion.h2 variants={titleV.artist} style={{ textShadow: "0 2px 8px #00000030" }} className="text-white text-xl text-center mt-2">
                  {song?.artists ? song.artists.map((artist) => artist.name).join(", ") : "Rick Astley"}
                </motion.h2>
              </motion.div>
              <motion.div>
                <Progress current={songProgress} setProgress={setSongProgress} controls={controls} />
              </motion.div>
              <motion.div>
                <motion.div variants={progressV.controls} className="flex justify-center items-center mt-8 gap-4">
                  ...
                  >
                    {isPlaying ? <IoPauseCircle className="w-full h-full" /> : <IoPlayCircle className="w-full h-full" />}
                  </PlayBackButton>
